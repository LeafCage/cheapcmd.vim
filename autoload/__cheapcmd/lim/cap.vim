...
function! s:_cnvvimkeycodes(str) "{{{
  try
    let ret = has_key(s:, 'disable_keynotation') ? a:str : __cheapcmd#lim#keynotation#decode(a:str)
    return ret
  catch /E117:/
...
"=============================================================================
"Main:
function! __cheapcmd#lim#cap#select(prompt, choices, ...) abort "{{{
...
function! __cheapcmd#lim#cap#keybind(binddefs, ...) abort "{{{
  let behavior = a:0 ? a:1 : {}
  let bindacts = s:_get_bindacts(a:binddefs, function(get(behavior, 'expand') ? 's:_expand_keycodes' : 's:_noexpand'))
  return __cheapcmd#lim#cap#keymappings(bindacts, behavior)
endfunction
"}}}
...
function! __cheapcmd#lim#cap#keymappings(keymappings, ...) abort "{{{
  let behavior = a:0 ? a:1 : {}
